...
goToPageAndRemove(BuildContext context, Widget screen) {
...
goToPage(BuildContext context, Widget screen) {
  Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => screen));
...
goToPageAndWaitForReturn(BuildContext context, Widget screen,
    Function onReturnedSuccessfully) {
  Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => screen)).then((value) {
    if (value == true) {
      onReturnedSuccessfully();
    }
  });
}

sendMail(BuildContext context , String email , String generatedPass ) async {
  String username = 'stem4app@gmail.com';
  String password = 'StemApp2019';
...
//      ..bccRecipients.add(Address('bccAddress@example.com'))
    ..subject = 'Your Bu-United Account Password!'
    ..html = "<h1>Welcome to BU United</h1>\n<h3>Thanks for joining us.</h3>"
        "\n<p>You've signed up to bu-united application with this Email: $email, And you password is: $generatedPass.</p>"
        "\n\n<p>Best Wishes</p>.\n<p>BU UNITED team.</p></p>";

  try {
    final sendReport = await send(message, smtpServer);
    print('Message sent: ' + sendReport.toString());
    snackBar(context, title:  'Msg Send successfully');
...
createToast(String msg, bool isGoodThing){
  Fluttertoast.showToast(
      msg: msg,
      toastLength: Toast.LENGTH_SHORT,
      gravity: ToastGravity.BOTTOM,
      timeInSecForIosWeb: 1,
      backgroundColor: (isGoodThing) ? Colors.green.withOpacity(0.7) :Colors.red.withOpacity(0.7),
...
class CustomDialog extends StatelessWidget {
  ...
  CustomDialog({required this.title, required this.text});
        ...
        child: Column(
          ...
          children: <Widget>[
            GestureDetector(
              ...
              child: Container(padding: EdgeInsets.all(16), alignment: Alignment.centerRight, child: Icon(Icons.close, color: textSecondaryColor)),
            ),
            Text(title, style: boldTextStyle(color: textPrimaryColor, size: 24)),
            16.height,
            Padding(
              padding: const EdgeInsets.only(left: 16, right: 16),
              child: Text(text, style: secondaryTextStyle(color: textSecondaryColor , size: 16 ) , textAlign: TextAlign.center,),
            ),
            16.height,
            appSmallButton(label: "Ok", onTap: ()=> finish(context), btnWidth: 100),
            30.height,
          ],
        ),
      ),
...
Widget itemList( Widget icon, String title , Function function) {
  return Row(
    children: [
      icon,
      10.width,
